...
カード情報は `cards.db` というSQLiteデータベースファイルに保存されます（Python版と同じデータベースを使用可能）。
...
- **Node.js版**: PC/SC経由でリーダーにアクセス（macOSでより安定）
- **Python版**: nfcpyライブラリを使用（Linuxでより安定）

両方のバージョンは同じデータベース（`cards.db`）を使用するため、どちらのバージョンで読み取ったカードも同じデータベースに保存されます。
...
- `cards.db`: データベースファイル（自動生成、Python版と共有可能）
